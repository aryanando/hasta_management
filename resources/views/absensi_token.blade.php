        ...
        function generateDataTable() {
            ...
            $.get("{{ url('') }}/get-absensi", function(data, status) {
                ...
                data.data.forEach(element => {
                    ...
                    rowCount++;
                    console.log(element);
                    var event = new Date(element.check_in);
                    var shiftCheckIn = element.shifts.check_in;
                    const d = new Date();
                    var shiftCheckInDate = new Date(
                    `2024-${d.getMonth()+1}-${d.getDate()} ${shiftCheckIn}`);
                    var eventEnd = new Date(element.check_out);
                    if (rowCount < 6) {
                        if (firstRow == 0) {
                            console.log();
                            firstRow++;
                            div.innerHTML +=
                                `<div class="row">
                                    <div class="col-6 text-success h5">${element.users.name}</div>
                                    <div class="col-2 text-success h5">${event.toLocaleTimeString('id-ID')}</div>
                                    <div class="col-2 text-success h5">${element.check_out == null ? '-' : eventEnd.toLocaleTimeString('id-ID')}</div>
                                    <div class="col-2 text-success h5">${element.check_out == null ? 'Masuk' : 'Pulang'}</div>
                                </div>`;
                        } else {
                            div.innerHTML +=
                                `<div class="row">
                                <div class="col-6 h6">${element.users.name}</div>
                                <div class="col-2 h6">${event.toLocaleTimeString('id-ID')}</div>
                                <div class="col-2 h6">${element.check_out == null ? '-' : eventEnd.toLocaleTimeString('id-ID')} </div>
                                <div class="col-2 text-success">${element.check_out == null ? 'Masuk' : 'Pulang'}</div>
                            </div>`
                        }
                    ...
                    // console.log(
                    //     `${event.getTime()} > ${shiftCheckInDate.getTime()} = ${shiftCheckInDate.getTime() < event.getTime()}`
                    //     );
                    ...
                    if (event.getTime() > shiftCheckInDate.getTime()) {
                        divLateData.innerHTML +=
                            `<li>
                                    <div class="row">
                                        <div class="col-8" style="font-size:10px">${element.users.name}</div>
